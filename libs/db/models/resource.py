...
class Resource(Base):
    ...
    parent_id: Mapped[Optional[UUID]] = mapped_column(ForeignKey("resources.id"), nullable=True, index=True)
    owner_principal_id: Mapped[UUID] = mapped_column(ForeignKey("principals.id"), index=True)
    space_kind: Mapped[Optional[str]] = mapped_column(String(50), nullable=True) # personal|team|system
    subspace_kind: Mapped[Optional[str]] = mapped_column(String(50), nullable=True) # official|staging|custom
...
class ResourceEdge(Base):
    ...
    src_resource_id: Mapped[UUID] = mapped_column(ForeignKey("resources.id"), primary_key=True, index=True)
    dst_resource_id: Mapped[UUID] = mapped_column(ForeignKey("resources.id"), primary_key=True, index=True)
    edge_type: Mapped[str] = mapped_column(String(100), primary_key=True) # contains|composes|references|...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
...
class ResourceVersion(Base):
    ...
    __table_args__ = (
        Index("ix_resource_versions_tenant_res_created", "tenant_id", "resource_id", "created_at"),
    )
...
class ResourceRef(Base):
    ...
    tenant_id: Mapped[UUID] = mapped_column(ForeignKey("tenants.id"), primary_key=True)
    resource_id: Mapped[UUID] = mapped_column(ForeignKey("resources.id"), primary_key=True, index=True)
    ref_name: Mapped[str] = mapped_column(String(255), primary_key=True) # main, staging, etc.
