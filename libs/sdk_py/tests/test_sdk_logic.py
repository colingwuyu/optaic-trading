...
def mock_httpx_client():
    mock = AsyncMock()
    mock.request.return_value = MagicMock(status_code=200, json=lambda: {"id": "test-id"})
    return mock
...
class TestPipelinesClient:
    ...
    @pytest.mark.asyncio
    async def test_create_instance_full_config(self, platform_client, mock_httpx_client):
        def_id = uuid4()
        parent_id = uuid4()
        schedule = {"cron": "* * * * *"}
...
class TestExperimentsClient:
    ...
    async def test_create_and_run(self, platform_client, mock_httpx_client):
        ...
        call_args_create = mock_httpx_client.request.call_args_list[-1] # Most recent call if sequential, but here separate
