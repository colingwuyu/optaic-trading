...
async def test_validate_no_bundle_returns_ok_report(
    engine, mock_db, context, resource_id
):
    """Test that if no bundle exists, we get a passing report."""
    with patch(
        "optaic.guardrails.storage.ContractBundleStore.get_active_bundle",
        new_callable=AsyncMock,
    ) as mock_get:
        mock_get.return_value = None

        with patch(
            "optaic.guardrails.runtime.engine.record_activity_with_outbox",
            new_callable=AsyncMock,
        ) as mock_emit:
            with patch(
                "optaic.guardrails.storage.ValidationReportStore.insert_report",
                new_callable=AsyncMock,
            ):
...
async def test_validate_official_blocks_on_error(
    engine, mock_db, context, sample_bundle, mock_registry, resource_id
):
...
async def test_validate_staging_warns_on_error(
    engine, mock_db, sample_bundle, mock_registry, resource_id
):
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                ...
                assert (
                    report.enforced_as == "warn"
                )  # Policy overrides block hint in staging
