...
def _check_engines(config: RuntimeConfig) -> tuple[list[DiagnosticItem], dict[str, dict]]:
    ...
    if config.prefect.enabled:
        if prefect_mode == "local":
            ...
            items.append(DiagnosticItem(
                ...
                details=f"DB: {db_path}" + (" (exists)" if db_path.exists() else " (not created)"),
            ))
        else:
            items.append(DiagnosticItem(
                name="Prefect Engine",
...
def _check_services(config: RuntimeConfig) -> tuple[list[DiagnosticItem], dict[str, dict]]:
    ...
    service_checks = [
        ("prefect-server", config.prefect.enabled and config.prefect.is_local_mode, "127.0.0.1", config.prefect.port),
        ("mlflow", config.mlflow.enabled and config.mlflow.is_local_mode, "127.0.0.1", config.mlflow.port),
    ]
    ...
    for name, enabled, host, port in service_checks:
        ...
        if health.status == "up":
            ...
            items.append(DiagnosticItem(
                ...
                details=f"Last log lines:\n{log_hint}" if log_hint else "No log file found",
                suggestion=f"Check log file: {log_file}" if log_file.exists() else "Service may not have started",
            ))
...
def _check_stale_pids(data_dir: Path, *, auto_clean: bool = True) -> list[DiagnosticItem]:
